...
async function validate_response(inputString: responseData, from: string, req: Request, res: Response) {
...
async function send_calendar_msg(to: string, text?: string) {
    ...
    try {
        const response = await axios.post("https://api.gupshup.io/wa/api/v1/msg", postData, axiosConfig);
        return response;
    } catch (error) {
        throw error;
    }
...
async function sendMessage(to: string, text: string, openaiApiKey: string) {
    try {
        ...
        const messages = [
            { role: 'system', content: 
            `Process the following statement "${text}" and return the response in the following JSON format:
